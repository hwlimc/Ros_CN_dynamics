...
colnames(wg1)<-c('Snr','direction','year','age','wid','wid.e','wid.l','wg','wg.mn','wg.mx','wg.e','wg.l')
wgg<-merge(wg1,aggregate(wid~direction+Snr,wg1,sum),all.x=T,by=c('direction','Snr'))
head(wgg)
write.table(wgg,"../Ring_wg_wid_2018.txt",sep="\t",quote=FALSE,row.names=FALSE)
